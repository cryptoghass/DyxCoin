...
uint256 hashGenesisBlock("0x000007718d1e5231ef42f99e80a3e09405784415471bd4a314b91fab1bf12c61");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DynamicsCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DynamicsCoin Signed Message:\n";
...
{
    // DynamicsCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // DynamicsCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "dynamic genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DynamicsCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DynamicsCoinMiner : ProcessBlock, block not accepted");
    }
